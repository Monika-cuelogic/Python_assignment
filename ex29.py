...
def playGame():
  ...
  while True:
    ...
    if player1:
      print(" Player 1 Won !!!!!!")
      winCount = playHistory['player1']
      playHistory['player1'] = winCount + 1
      print(f" Play Board Score: {playHistory} ")
      break
    player2 = putToken('o')
    if player2:
      print(" Player 2 Won !!!!!!")
      winCount = playHistory['player2']
      playHistory['player2'] = winCount + 1
      print(f" Play Board Score: {playHistory} ")
      break

def continueGame():
  global playHistory
  playHistory = {'player1': 0, 'player2': 0}

  while True:
    playGame()
    choice = input(" Do you want to Continue? Press 'y' else 'n': ")
    if choice == 'n':
      break
